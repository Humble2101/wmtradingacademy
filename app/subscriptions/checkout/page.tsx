      ...
      <div className="min-h-screen flex flex-col bg-bg">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <Link href="/subscriptions" className="btn-gold px-6 py-3 rounded-xl text-sm">Browse Plans</Link>
          </div>
        </div>
      </div>
    )
        ...
        <div className="flex-1 flex items-center justify-center px-4 py-20">
          <div className="card-dark p-10 max-w-md w-full text-center">
            ...
            <p className="text-[#8BA3CC] text-sm leading-relaxed mb-8">
              Your bank transfer has been logged. Our team will verify and activate your <strong className="text-white">{plan.name}</strong> plan within <strong className="text-gold">2–4 hours</strong>. You'll receive a confirmation notification.
            </p>
            <div className="bg-bg border border-border rounded-xl p-4 mb-8 text-left space-y-2">
              <div className="flex justify-between text-sm"><span className="text-[#4A6A99] font-mono-dm text-xs">Plan</span><span>{plan.name} ({type})</span></div>
              <div className="flex justify-between text-sm"><span className="text-[#4A6A99] font-mono-dm text-xs">Amount</span><span className="text-gold font-bold">{formatNaira(plan.price)}</span></div>
              <div className="flex justify-between text-sm"><span className="text-[#4A6A99] font-mono-dm text-xs">Payment Code</span><span className="font-mono-dm text-xs">{payCode}</span></div>
            </div>
            <Link href="/dashboard" className="btn-gold w-full py-3.5 text-sm rounded-xl block text-center">Go to Dashboard →</Link>
          </div>
        </div>
        <Footer />
      </div>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-bg">
      ...
      <div className="max-w-4xl mx-auto px-4 py-12 w-full">
        ...
        <Link href="/subscriptions" className="inline-flex items-center gap-1.5 text-sm text-[#8BA3CC] hover:text-white mb-8 transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="card-dark p-6 sticky top-24">
              ...
              <div className={`inline-block px-3 py-1 rounded-full text-[11px] font-mono-dm font-bold mb-3 ${plan.featured ? 'bg-gold-grad text-bg' : 'bg-surface text-[#8BA3CC]'}`}>
              ...
              <div className="mt-6 p-3 bg-gold/5 border border-gold/20 rounded-xl">
                <p className="text-[11px] text-gold font-mono-dm">🔒 Secure checkout · SSL encrypted</p>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <p className="text-[#8BA3CC] text-sm">Choose how you'd like to pay for your {plan.name} plan.</p>

            {/* Method selector */}
            <div className="grid grid-cols-2 gap-4">
              {[
                { id: 'card', icon: <CreditCard size={20} className="text-accent-cyan"/>, bg: 'bg-accent-cyan/10', label: 'Card Payment', sub: 'Stripe · Instant' },
                { id: 'bank', icon: <Building2 size={20} className="text-gold"/>, bg: 'bg-gold/10', label: 'Bank Transfer', sub: 'Manual · 2–4 hrs' },
              ].map(m => (
                <button key={m.id} onClick={() => setPayMethod(m.id as any)}
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div className={`ml-auto w-4 h-4 rounded-full border-2 flex items-center justify-center ${payMethod === m.id ? 'border-gold' : 'border-border-2'}`}>
            ...
            {payMethod === 'card' && (
              <div className="card-dark p-6 space-y-4">
                ...
                <div>
                  <label className="block font-mono-dm text-[11px] tracking-widest text-[#8BA3CC] uppercase mb-2">Card Number</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block font-mono-dm text-[11px] tracking-widest text-[#8BA3CC] uppercase mb-2">Expiry</label>
                    <input className="input-dark" placeholder="MM / YY" maxLength={7} />
                  </div>
                  <div>
                    <label className="block font-mono-dm text-[11px] tracking-widest text-[#8BA3CC] uppercase mb-2">CVV</label>
                    <input className="input-dark" placeholder="•••" maxLength={4} />
                  </div>
                </div>
                <div>
                  <label className="block font-mono-dm text-[11px] tracking-widest text-[#8BA3CC] uppercase mb-2">Cardholder Name</label>
                ...
                <button onClick={handleCardPay} disabled={loading}
                  ...
                  {loading ? <><Loader2 size={16} className="animate-spin"/> Processing...</> : `Pay ${formatNaira(plan.price)} →`}
                </button>
              </div>
            )}
            ...
            {payMethod === 'bank' && (
              <div className="card-dark p-6 space-y-5">
                ...
                <p className="text-[#8BA3CC] text-sm">Transfer exactly <strong className="text-gold">{formatNaira(plan.price)}</strong> to the account below, then click "I've Sent Payment".</p>
                  ...
                  {[
                    ...
                    <div key={row.k} className="flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`font-mono-dm text-xs ${row.highlight ? 'text-gold font-bold' : 'text-white'}`}>{row.v}</span>
                        {row.copy && (
                          <button onClick={() => copy(row.v, row.k)} className="p-1.5 rounded-lg bg-surface hover:bg-border-2 transition-colors">
                ...
                <div className="bg-gold/5 border border-gold/20 rounded-xl p-4">
                  <p className="text-[11px] text-gold font-mono-dm leading-relaxed">
                    ⚠ <strong>Important:</strong> You MUST include the Payment Code (<strong>{payCode}</strong>) in the "Narration" or "Description" field of your bank transfer. This is how we identify your payment.
                  </p>
                </div>
                <button onClick={handleBankConfirm} disabled={loading}
                  className="btn-gold w-full py-4 text-sm rounded-xl disabled:opacity-60 flex items-center justify-center gap-2">
                  {loading ? <><Loader2 size={16} className="animate-spin"/> Recording...</> : "I've Sent the Payment →"}
                </button>
              </div>
            )}
          </div>
